...
function defResolveHandler(x) {
  return x;
}
function defRejectHandler(r) {
  throw r;
}
 ...
 *
 * @param {Function} resolver a function that accepts `fn(err, ...){}` as signature
...
function Promise(resolver) {
  if (resolver && typeof(resolver) !== 'function') throw new TypeError('need to call with new');
  if (!(this instanceof Promise)) throw new TypeError('need to call with new');
  ...
  try {
    resolver(
      function() { p.fulfill.call(p, arguments); },
      function(r) { p.reject(r) }
    );
  } catch (e) {
    p.reject(e);
  }
 ...
 * @return {Promise} this
 */
Promise.prototype.catch = Promise.prototype.end = function (onReject) {
...
Promise.fulfilled = function fulfilled() { var p = new this; p.fulfill.apply(p, arguments); return p; };
Promise.rejected = function rejected(reason) { return new this().reject(reason); };
...
/* This id for V8 tests */
Promise.resolve = Promise.accept = Promise.fulfilled;
Promise.reject = Promise.rejected;
Promise.defer = Promise.deferred;
Promise.all = function static_all (arr) { return Promise.fulfilled().all(arr); };
/* end of V8 tests */
